...
const DATASOURCE = {
    MONGOOSE: new (require("./services/MongooseService"))()
}
...
module.exports = class PermissionManager {
    ...
    constructor({ me, dataSource }) {
        ...
        this._me = me;
        this._dataSource = DATASOURCE[dataSource.name];
        this._dataSource.setInstance(dataSource.instance);
    }

    async setRole(id, ressource, roles) {
        return this._dataSource.save({id, ressource, roles});
    }

    async check(ressource, actions, throwError = true) {
        const rules = getRulesOf(ressource.name, this._me);
        if(!rules)
            throw new Error(`No rules found for ressource '${ressource}'.`);

        const results = await this._dataSource.find(this._me.id, ressource);
